...
- do not assume the current contract still supports legacy `EOA` writes
...
- if a prior agent used `ManagerForwardCall` or a generic send path for a resonance `Get*` method, say that the call path is wrong before interpreting pair or queue state
- if the user pasted an old `EOA` or `ManagerForwardCall` receipt, explain that it belongs to the pre-`v2.0.0` legacy path
